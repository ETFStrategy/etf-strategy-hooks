...
contract CLFullRangeHook is CLBaseHook {
    ...
    function addLiquidity(
        ...
        StandardToken(payable(pool.liquidityToken)).mint(params.to, liquidity);
    ...
    function removeLiquidity(
        ...
        StandardToken liquidityToken = StandardToken(
            payable(poolInfo[poolId].liquidityToken)
        );
        ...
        liquidityToken.burnFrom(msg.sender, params.liquidity);
    }
    ...
    function _beforeInitialize(
        ...
        if (key.parameters.getTickSpacing() != 60)
        ...
        address poolToken = address(
            new StandardToken(tokenSymbol, tokenSymbol)
        );
    ...
    function _modifyPosition(
        ...
        delta = abi.decode(
            vault.lock(abi.encode(CallbackData(msg.sender, key, params))),
            (BalanceDelta)
        );
    }
    ...
    function _removeLiquidity(
        ...
        uint256 liquidityToRemove = FullMath.mulDiv(
            ...
            StandardToken(payable(pool.liquidityToken)).totalSupply()
        );
