...
import {CLFullRangeHook} from "../../src/hooks/CLFullRangeHook.sol";
import {StandardToken} from "../../src/contracts/StandardToken.sol";
...
contract CLFullRangeHookTest is Test, Deployers, DeployPermit2 {
    ...
    function setUp() public {
        ...
        fullRange = new CLFullRangeHook(poolManager);
...

        {
            ...
            key = PoolKey({
                ...
                fee: 3000,
                parameters: bytes32(
                    uint256(fullRange.getHooksRegistrationBitmap())
                ).setTickSpacing(60)
            });
            id = key.toId();
        }
        ...
        fullRange.addLiquidity(
            CLFullRangeHook.AddLiquidityParams({
    ...
    function test_RevertIfWrongTickSpacing() public {
        PoolKey memory wrongKey = PoolKey({
            ...
            poolManager: poolManager,
            fee: 3000,
            parameters: bytes32(uint256(fullRange.getHooksRegistrationBitmap()))
                .setTickSpacing(61)
        });
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                abi.encodeWithSelector(
                    CLFullRangeHook.TickSpacingNotDefault.selector
                ),
    ...
    function test_RevertIfNoPool() public {
        vm.expectRevert(CLFullRangeHook.PoolNotInitialized.selector);
        fullRange.addLiquidity(
            CLFullRangeHook.AddLiquidityParams({
                currency0: key.currency0,
                currency1: key.currency1,
        ...
        fullRange.removeLiquidity(
            CLFullRangeHook.RemoveLiquidityParams({
                currency0: key.currency0,
                currency1: key.currency1,
                fee: key.fee,
                parameters: key.parameters,
    ...
    function test_RevertIfTooMuchSlippage() public {
        ...
        vm.expectRevert(CLFullRangeHook.TooMuchSlippage.selector);
        fullRange.addLiquidity(
            CLFullRangeHook.AddLiquidityParams({
                currency0: key.currency0,
                currency1: key.currency1,
    ...
    function test_AddLiquidity() public {
        ...
        uint256 liquidityTokenBalance = StandardToken(payable(liquidityToken))
            .balanceOf(address(this));

        assertEq(
            poolManager.getLiquidity(id),
            liquidityTokenBalance + MINIMUM_LIQUIDITY
        );
    ...
    function testFuzz_AddLiquidity(uint256 amount) public {
        ...
        if (amount <= MINIMUM_LIQUIDITY) {
            vm.expectRevert(
                CLFullRangeHook.LiquidityDoesntMeetMinimum.selector
            );
            fullRange.addLiquidity(
                CLFullRangeHook.AddLiquidityParams({
                    currency0: key.currency0,
            ...
            (bool hasAccruedFees, address liquidityToken) = fullRange.poolInfo(
                id
            );
            uint256 liquidityTokenBalance = StandardToken(
                payable(liquidityToken)
            ).balanceOf(address(this));
    ...
    function test_RevertIfNoLiquidity() public {
        ...
        StandardToken(payable(liquidityToken)).approve(
            address(fullRange),
            type(uint256).max
        );
    ...
    function test_RemoveLiquidity() public {
        ...
        assertEq(
            poolManager.getLiquidity(idWithLiq),
            liquidityTokenBalance + MINIMUM_LIQUIDITY
        );
        assertEq(
            StandardToken(payable(liquidityToken)).balanceOf(address(this)),
            99 ether - MINIMUM_LIQUIDITY + 5
        );
        assertEq(
            keyWithLiq.currency0.balanceOf(address(this)),
            prevBalance0 + 1 ether - 1
        );
        assertEq(
            keyWithLiq.currency1.balanceOf(address(this)),
            prevBalance1 + 1 ether - 1
        );
        assertEq(hasAccruedFees, false);
    }
    ...
    function testFuzz_RemoveLiquidity(uint256 amount) public {
        ...
        if (
            ...
            uint256 prevLiquidityTokenBalance = StandardToken(
                payable(liquidityToken)
            ).balanceOf(address(this));
